function gradient_ascent(use_sim, position, heading, xvals, yvals,zvals, grad_f_x, grad_f_y)
    ...
    step_size = 0.05; % Step size, smaller step size leads to more precise ascent
    decay = 1;
    threshold = 15; % Gradient magnitude threshold for stopping  
    ...
    while true
        ...
        x_index = round(position(1), 1);
        y_index = round(position(2), 1);
        % abs(xvals-x_index) < 0.000001
        % abs(xvals-x_index) < 0.000001
        [x_r, x_c] = find((abs(xvals-x_index) < 0.000001) == 1, 1);
        [y_r, y_c] = find((abs(yvals-y_index) < 0.000001) == 1, 1);
        grad_x = grad_f_x(x_c, y_r);
        grad_y = grad_f_y(x_c, y_r);
        grad = [grad_x; grad_y];
